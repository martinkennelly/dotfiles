...
if [ "$DISTRO" == "DEBIAN" ]; then
  ...
  sudo yum install -y gcc-c++ ncurses lua-devel ncurses-devel python-devel git python3 perl-devel perl-ExtUtils-Embed cmake python3-devel npm
fi
...
if [ "$DISTRO" == "REDHAT" ]; then
  PYDIR="/usr/lib64/python3.6/config"
elif [ "$DISTRO" == "DEBIAN" ]; then
  PYDIR="/usr/lib/python3.5/config"
fi
